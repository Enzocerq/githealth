...
import { getRepositoriesData } from "@/app/actions/metrics";
...
export default async function RepositoriosPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Seus repositorios ({repositories.length})</CardTitle>
        </CardHeader>
              ...
              <tbody className="divide-y">
                ...
                {repositories.map((repo) => (
                  <tr key={repo.id} className="hover:bg-muted/40 transition-colors">
                    ...
                    <td className="px-4 py-3 text-right tabular-nums">
                      {formatNumber(repo.prs)}
                    </td>
                    <td className="px-4 py-3 text-right tabular-nums">
                      {formatNumber(repo.mergedPRs)}
                    </td>
                    <td className="px-4 py-3 text-right tabular-nums">
                      {formatPercent(repo.participation)}
                    </td>
